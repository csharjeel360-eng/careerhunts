...
export default async function JobsPage({ searchParams }: JobsPageProps) {
  ...
  const skill = Array.isArray(resolvedSearchParams.skill) ? resolvedSearchParams.skill[0] : resolvedSearchParams.skill || ''
  ...
  const filteredJobs = normalizedSkill
    ? jobs.filter((job: any) => {
        const requiredSkills = Array.isArray(job.requiredSkills) ? job.requiredSkills : []
        return requiredSkills.some((item: string) => item.toLowerCase().includes(normalizedSkill))
      })
    : jobs
  ...
  const allOpportunities = filteredJobs
    .map((job: any) => ({
      ...job,
      type: 'local',
  ...
  const jobCategories = Array.from(new Set(filteredJobs.map((job: any) => job.category).filter(Boolean))).sort()
  const jobCountries = Array.from(new Set(filteredJobs.map((job: any) => job.country).filter(Boolean))).sort()
  const jobCities = Array.from(new Set(filteredJobs.map((job: any) => job.city).filter(Boolean))).sort()
  ...
  const cacheKey = `careerhunt-jobs-cache:${[keyword, category, country, city, company, skill].map((value) => (value || 'all').toLowerCase()).join('|')}`
      ...
      <div className="container relative mx-auto px-4 py-14 sm:py-16">
        <div className="mx-auto max-w-4xl text-center">
          <div className="inline-flex items-center rounded-full border border-slate-200 bg-white/80 px-4 py-2 text-sm font-medium text-slate-700 shadow-sm backdrop-blur">
            ...
            UAE jobs, Dubai jobs, Abu Dhabi jobs
          </div>
          <h1 className="mt-6 text-4xl font-semibold tracking-tight text-slate-900 sm:text-5xl">
            Find UAE jobs in Dubai, Abu Dhabi, Sharjah and beyond
          </h1>
          <p className="mx-auto mt-4 max-w-2xl text-base leading-7 text-slate-600">
            Search for entry-level, mid-career, and specialist roles across the UAE with filters for city, category, company, and skill.
          </p>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-1">
          <div className="rounded-[1.5rem] border border-slate-200 bg-white/90 p-5 shadow-sm backdrop-blur">
            ...
            <p className="mt-2 text-2xl font-semibold text-slate-900">Refine your UAE search</p>
          </div>
        </div>
        ...
        <div className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_-24px_rgba(15,23,42,0.28)]">
          <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-slate-900">Refine your UAE job search</p>
              <p className="text-sm text-slate-500">Use one or more filters to find Dubai jobs, Abu Dhabi jobs, and other UAE opportunities quickly.</p>
            </div>
